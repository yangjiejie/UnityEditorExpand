...
public class FolderTreeEditor  
{
    ...
    public void RenameFolder(FolderNode node)
    {
        if (node == null) return;
        ...
        while (list.Count > 0)
        {
            ...
            var combinePath = Path.Combine(Environment.CurrentDirectory, "Assets", top.originUnityPath).ToLinuxPath();
            var combineTargetPath = Path.Combine(Environment.CurrentDirectory, "Assets", top.NowUnityPath).ToLinuxPath();
            if (Directory.Exists(combinePath))
            {
                AssetDatabase.MoveAsset(combinePath.ToUnityPath(), combineTargetPath.ToUnityPath());
            }
            for(int i = top.Children.Count - 1; i >= 0; --i)
            {
                list.Add(top.Children[i]);
            }
        }
        list.Clear();
    }
